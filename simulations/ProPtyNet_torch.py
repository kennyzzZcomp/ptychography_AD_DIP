...
def make_probe_init(cfg: Cfg, rr, support):
    ...
    默认 z_probe_init = 1.3 * z_probe，得到的 P0 相对真值探针的复相对误差
    （消去全局复因子后）= 0.217，|相关| = 0.976。作为对照: 随机复场 = 1.000，
    全 1 平顶圆盘 = 0.693。即这个"故意差 30%"的初值其实相当强。
    """
...
def run_net(cfg: Cfg):
    ...
    if cfg.probe_mode in ("pixel", "inr"):
        ...
        # 默认 z_probe_init=1.3*z_probe 时约 0.217（随机复场=1.000，全1平顶盘=0.693），
        # 即这个"故意差 30%"的初值其实相当强 —— 改 --z-probe-init 可以扫这条轴。
