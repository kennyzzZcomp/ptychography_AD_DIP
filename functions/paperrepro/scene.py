...
from functions.paperrepro.evaluate import illum_roi
...
def banner(cfg: Cfg, sc):
    ...
    print(f"[scene] 评价 ROI = 照明覆盖区 {rs.stop-rs.start}×{cs.stop-cs.start}"
          f"（评价函数 paperrepro.evaluate.evaluate，三种算法同一个）")
